    ...
    def convert(salary)
      {
        :id                       => salary.id,
        :date                     => @options[:date],
        :created_at               => salary.created_at.strftime("%d.%m.%Y"),
        :from                     => salary.from,
        :to                       => salary.to,
        :title                    => salary.title,
        :yearly_salary            => salary.reference.yearly_salary.to_view,
        :value                    => salary.gross_pay.to_view,
        :gross_pay                => salary.gross_pay.to_view,
        :employee_value_total     => salary.employee_value_total.to_view,
        :employer_value_total     => salary.employer_value_total.to_view,
        :net_pay                  => salary.net_salary.to_view,
